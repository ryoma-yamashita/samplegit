$(function () {
  ...
  function displayResult(res) {
    ...
    if (res[0].items == undefined || res[0].items.length == 0) {
      // エラーメッセージの定義
      let noResult = '<div class = "message">検索結果が見つかりませんでした。<br>別のキーワードで検索して下さい。</div>';
      ...
      $.each(res[0].items, function (index) {
        ...
        let listItems = `<li class = "lists-item"><div class = "list-inner"><p>タイトル：${title}</p><p>作者：${creator}</p><p>出版社：${publisher}</p><a href = "${id}" target="_blank">書籍情報</a></div></li>`;
        // listsクラスに追加し表示させる処理
  ...
  function displayError(err) {
    ...
    let errorConnect = '<div class = "message">正常に通信できませんでした。<br>インターネットの接続を確認してください。</div>';
    let errorText = '<div class = "message">検索ワードが有効ではありません。<br>1文字以上で検索してください。</div>';
    let errorMessage = '<div class = "message">予期せぬエラーが起きました。<br>再読み込みを行ってください。</div>';
